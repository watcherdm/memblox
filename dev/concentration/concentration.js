    ...
    //ctxt.drawImage(imageObjs[1], 0, 0);
    //ctxt.drawImage(bgImage, 0,0);
    
    var cards = [];
    for (i=0; i < 4; i++)
    {
        var tempImage = new Image();
        tempImage.src = 'back.png';
        tempImage.setAttribute('class', 'card');
        var newdiv = document.createElement('div');
        newdiv.setAttribute('id', 'card' + i);
        newdiv.setAttribute('class', 'unflip');
        newdiv.setAttribute('data-value', imageUrls[i % 2]);
        //newdiv.setAttribute('height', '100');
        //newdiv.setAttribute('width', '80');
        newdiv.appendChild(tempImage);
        cards[i] = newdiv;
	...
	{
        $('.flip').children().animate({width: '0px', height:'96px'}, 
            {duration:'fast', complete:function(){$(this).attr('src', 'back.png')}
            });
        $('.flip').children().animate({width: '71px', height:'96px'}, 200);
	...
	function oninitial() {
		$('.unflip').click(
			function(){
                ...
                {
                    var curImg = $(this).children().first();
                    curImg.animate({width: '0px', height:'96px'}, 
                        {duration:'fast', complete:function(){curImg.attr('src', curImg.parent().attr('data-value'))}
					});
                    curImg.animate({width: '71px', height:'96px'}, 200);
